...
- (void)fetchProfileAttrbuteMetadata {
    ...
    [[PWMSGAttributeManager sharedManager] fetchProfileAttributeMetadataWithCompletion:^(NSArray *attributes, NSError *error) {
        if (error) {
...
- (void)fetchProfileAttributesForDevice {
    [[PWMSGAttributeManager sharedManager] fetchProfileAttributesWithCompletion:^(NSDictionary *attributes, NSError *error) {
        if (error) {
            [PubUtils displayError:error];
        } else {
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (IBAction)saveProfileAttribute:(id)sender {
    ...
    [[PWMSGAttributeManager sharedManager] updateProfileAttributes:self.userAttributes completion:^(NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (error) {
                [PubUtils displayError:error];
            } else {
