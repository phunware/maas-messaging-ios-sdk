...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshMKCircles) name:PWLMEnterGeoZoneNotificationKey object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshMKCircles) name:PWLMExitGeoZoneNotificationKey object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshMKCircles) name:PWLMMonitoredGeoZoneChangesNotificationKey object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshMKCircles) name:PWLMModifyGeoZonesNotificationKey object:nil];
}
...
- (void)refreshMonitoredRegions
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0),^{
        ...
        @autoreleasepool {
            ...
            self.monitoredLocation = [NSSet setWithArray:[PWMessaging monitoredGeozones]];
            
            NSArray *coreLocationMonitoredRegions = [PWMessaging monitoredGeozones];
...
- (void)refreshMKCircles
{
    dispatch_async(dispatch_get_main_queue(),^{
        ...
        @autoreleasepool {
            ...
            self.monitoredLocation = [[NSSet alloc] initWithArray:[PWMessaging monitoredGeozones]];
            
            NSMutableSet *nonMonitored  = [[NSMutableSet alloc] initWithArray:[PWMessaging geozones]];
            ...
            for (PWMSGGeozone* geozone in self.monitoredLocation) {
            ...
            for (PWMSGGeozone* geozone in self.nonMonitoredLocation) {
...
- (MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(id<MKAnnotation>)annotation
{
    ...
    if ([annotation isKindOfClass:[LPMapAnnotation class]]) {
        ...
        if ([mapAnnotation.zone isKindOfClass:[PWMSGGeozone class]])
        {
            PWMSGGeozone *geozone = (PWMSGGeozone*) mapAnnotation.zone;
            if (geozone.inside) {
