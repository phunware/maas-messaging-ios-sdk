...
- (void)viewDidLoad {
    ...
    self.notificationNames = @[PWLMAddGeoZonesNotificationKey,
                                   PWLMModifyGeoZonesNotificationKey,
                                   PWLMDeleteGeoZonesNotificationKey,
                                   PWLMEnterGeoZoneNotificationKey,
                                   PWLMExitGeoZoneNotificationKey,
                                   PWLMMonitoredGeoZoneChangesNotificationKey
                                   ];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"LocationDetails"]) {
        id<PWMSGZone> zone = self.displayedZones[[self.tableView indexPathForSelectedRow].row];
...
- (void)refreshZoneManagerDisplayInformation {
    dispatch_async(dispatch_get_main_queue(), ^{
        self.zones = [PWMessaging geozones];
...
- (void)updateDisplayedZones {
    switch (self.selectedFilteringSegment) {
        ...
        case 1:
        {
            ...
            for (id<PWMSGZone> zone in self.zones) {
                if (zone.inside) {
...
-(void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    id<PWMSGZone> zone = self.displayedZones[indexPath.row];
